 ...
 */
SMTPConnection.prototype.send = function (envelope, message, done, awCustomCallback) {
    if (!message) {
        return done(this._formatError('Empty message', 'EMESSAGE'));
    }

    //if (!message || !message._readableState || !message._readableState.buffer || !message._readableState.buffer.length) {
    //    awCustomCallback(null);
    //} else {
    //    awCustomCallback(message._readableState.buffer[0]);
    //}
...
SMTPConnection.prototype._createSendStream = function (callback, awCustomCallback) {
    ...
    if (typeof awCustomCallback === "function") {
        var awCustomStream = new PassThrough();
        awCustomStream.on("readable", function () {
            var chunk;
            while ((chunk = awCustomStream.read())) {
                awCustomCallback(chunk.toString('binary'));//.replace(/\r?\n$/, ''));
            }
        }.bind(this));
        dataStream.pipe(awCustomStream);
    }
