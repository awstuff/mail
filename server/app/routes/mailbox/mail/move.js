...
module.exports = function (req, res) {
	...
	let targetMailbox = req.body.targetMailbox;

	if (isNaN(accountId) || accountId < 1 || !validate.stringNotEmpty(mailbox) || !validate.stringNotEmpty(targetMailbox) || isNaN(uid)) {
		res.json({
			success: false
		});
		return;
		...
		Account.getById(accountId, (err, account) => {
			...
			connectImap(req, account.imap, (errorMessage, success, connection) => {
				...
				connection.openBox(mailbox, false, (err, box) => {
					...
					connection.move(uid, targetMailbox, err => {
						connection.closeBox(() => {
							connection.end();
						});
